...
router.put('/album/:id', auth, async (context, next) => {
  await photo.updateAlbum(context.params.id, context.body.name, context.state.user)
  await next()
}, responseOK)
/**
 * 删除相册
...
router.post('/photo', auth, uplader.single('file'), async (context, next) => {
  ...
  await photo.add(context.state.user.id, `https://www.ksgxkai.top/${file.filename}`, id)
  await next()
}, responseOK)
/**
 * 删除相片
...
router.delete('/photo/:id', auth, async (context, next) => {
  ...
  if (p) {
    if (p.userId === context.state.user.id.toString()) {
